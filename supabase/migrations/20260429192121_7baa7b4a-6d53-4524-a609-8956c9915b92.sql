-- ═══════════════════════════════════════════════════════════════════════════
-- Onda A — Importação de Notas de Entrada
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 0. Mescla duplicatas existentes por CNPJ ───────────────────────────────
DO $$
DECLARE
  r RECORD;
  v_canonico uuid;
BEGIN
  FOR r IN
    WITH norm AS (
      SELECT id, created_at, length(coalesce(nome_razao_social,'')) AS len_nome,
             regexp_replace(coalesce(cpf_cnpj,''),'\D','','g') AS cnpj_norm
      FROM public.fornecedores
      WHERE cpf_cnpj IS NOT NULL AND cpf_cnpj <> ''
    ),
    dups AS (
      SELECT cnpj_norm FROM norm GROUP BY cnpj_norm HAVING count(*) > 1
    )
    SELECT n.cnpj_norm,
           array_agg(n.id ORDER BY n.len_nome DESC, n.created_at ASC) AS ids
    FROM norm n
    JOIN dups d ON d.cnpj_norm = n.cnpj_norm
    GROUP BY n.cnpj_norm
  LOOP
    v_canonico := r.ids[1];
    -- Reaponta todas as FKs para o canônico
    UPDATE public.notas_fiscais            SET fornecedor_id = v_canonico WHERE fornecedor_id = ANY(r.ids[2:]);
    UPDATE public.compras                  SET fornecedor_id = v_canonico WHERE fornecedor_id = ANY(r.ids[2:]);
    UPDATE public.financeiro_lancamentos   SET fornecedor_id = v_canonico WHERE fornecedor_id = ANY(r.ids[2:]);
    UPDATE public.pedidos_compra           SET fornecedor_id = v_canonico WHERE fornecedor_id = ANY(r.ids[2:]);
    UPDATE public.cotacoes_compra_propostas SET fornecedor_id = v_canonico WHERE fornecedor_id = ANY(r.ids[2:]);
    UPDATE public.nfe_distribuicao         SET fornecedor_id = v_canonico WHERE fornecedor_id = ANY(r.ids[2:]);
    -- produtos_fornecedores: tem ON DELETE CASCADE; mas pode haver UNIQUE(produto_id, fornecedor_id)
    -- por isso reaponta apenas onde não existe par já
    UPDATE public.produtos_fornecedores pf SET fornecedor_id = v_canonico
    WHERE pf.fornecedor_id = ANY(r.ids[2:])
      AND NOT EXISTS (
        SELECT 1 FROM public.produtos_fornecedores pf2
        WHERE pf2.produto_id = pf.produto_id AND pf2.fornecedor_id = v_canonico
      );
    -- Deleta restantes (CASCADE limpa produtos_fornecedores remanescentes)
    DELETE FROM public.fornecedores WHERE id = ANY(r.ids[2:]);
  END LOOP;
END $$;

-- ── 1. Coluna `origem` em fornecedores ─────────────────────────────────────
ALTER TABLE public.fornecedores
  ADD COLUMN IF NOT EXISTS origem text NOT NULL DEFAULT 'manual';

ALTER TABLE public.fornecedores
  DROP CONSTRAINT IF EXISTS chk_fornecedores_origem;

ALTER TABLE public.fornecedores
  ADD CONSTRAINT chk_fornecedores_origem
  CHECK (origem IN ('manual','import_xml_entrada','import_planilha','distdfe','api'));

-- Índice único por CNPJ normalizado
CREATE UNIQUE INDEX IF NOT EXISTS uq_fornecedores_cpf_cnpj_norm
  ON public.fornecedores ((regexp_replace(coalesce(cpf_cnpj,''), '\D', '', 'g')))
  WHERE cpf_cnpj IS NOT NULL AND cpf_cnpj <> '';

-- ── 2. Dedup em notas_fiscais ──────────────────────────────────────────────
CREATE UNIQUE INDEX IF NOT EXISTS uq_notas_fiscais_chave_acesso
  ON public.notas_fiscais (chave_acesso)
  WHERE chave_acesso IS NOT NULL AND chave_acesso <> '';

CREATE UNIQUE INDEX IF NOT EXISTS uq_notas_fiscais_entrada_sem_chave
  ON public.notas_fiscais (numero, serie, fornecedor_id, modelo_documento)
  WHERE tipo_operacao = 'entrada'
    AND fornecedor_id IS NOT NULL
    AND (chave_acesso IS NULL OR chave_acesso = '');

-- ── 3. RPC importar_nfe_entrada ────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.importar_nfe_entrada(jsonb);

CREATE OR REPLACE FUNCTION public.importar_nfe_entrada(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    ...
    IF v_fornecedor_id IS NULL THEN
      INSERT INTO public.fornecedores (
        tipo_pessoa, nome_razao_social, ativo, origem, observacoes
      ) VALUES (
        'J', 'FORNECEDOR DESCONHECIDO (importação)', true, 'import_xml_entrada',
  ...
  IF v_nota_id IS NULL THEN
    INSERT INTO public.notas_fiscais (
      tipo, tipo_operacao, modelo_documento, numero, serie, chave_acesso,
      ...
      'entrada', 'entrada', v_modelo, v_numero, v_serie, v_chave,
      ...
      coalesce(nullif(trim(v_nota->>'origem'),''), 'importacao_xml'),
  ...
  IF v_chave IS NOT NULL THEN
    ...
    SET status_manifestacao = 'ciencia_da_operacao',
        updated_at = now()
    WHERE chave_acesso = v_chave
      AND coalesce(status_manifestacao,'sem_manifestacao') = 'sem_manifestacao';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.importar_nfe_entrada(jsonb) TO authenticated, service_role;