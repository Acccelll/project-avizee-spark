...
BEGIN
  ...
  WHERE l.empresa_id = p_empresa_id
    AND (p_periodo_inicio IS NULL OR l.data_movimento >= p_periodo_inicio)
    AND (p_periodo_fim IS NULL OR l.data_movimento <= p_periodo_fim);

  WITH auto_manual AS (
    SELECT
      COUNT(*) FILTER (WHERE m.status = 'aprovado') AS aprovados,
  ...
  FROM (
    SELECT
      cb.id AS conta_id,
      COALESCE(cb.descricao, cb.agencia || '/' || cb.conta) AS conta_nome,
    ...
    WHERE l.empresa_id = p_empresa_id
      AND (p_periodo_inicio IS NULL OR l.data_movimento >= p_periodo_inicio)
      AND (p_periodo_fim IS NULL OR l.data_movimento <= p_periodo_fim)
    GROUP BY cb.id, cb.descricao, cb.agencia, cb.conta
  ) t;
