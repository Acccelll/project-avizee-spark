...
DECLARE
  ...
  v_origem        text;
BEGIN
  -- Bypass legítimo dos triggers de proteção durante importação
  ...
  -- Mapeia valores não suportados pelo chk_nf_origem
  ...
  IF v_chave IS NOT NULL THEN
    ...
    SET status_manifestacao = 'ciencia_da_operacao', updated_at = now()
    WHERE chave_acesso = v_chave
      AND coalesce(status_manifestacao,'sem_manifestacao') = 'sem_manifestacao';
  END IF;

  RETURN jsonb_build_object(
    'ok', true, 'nota_id', v_nota_id, 'fornecedor_id', v_fornecedor_id,
    'fornecedor_criado', v_fornecedor_criado, 'itens_inseridos', v_itens_inseridos,
