-- Atualiza salvar_orcamento para usar NULLIF nos campos com CHECK constraints
-- (frete_tipo, modalidade, status, pagamento) para evitar 23514 quando vier string vazia.
-- Também faz backfill de orçamentos antigos com texto livre em frete_tipo.
...
-- Backfill: move texto livre em frete_tipo para servico_frete
UPDATE public.orcamentos
SET servico_frete = COALESCE(servico_frete, frete_tipo),
    frete_tipo = NULL
WHERE frete_tipo IS NOT NULL
  AND frete_tipo NOT IN ('CIF','FOB','sem_frete');

-- Normalizar status legado 'confirmado' (caso exista) para 'pendente'
UPDATE public.orcamentos
SET status = 'pendente'
