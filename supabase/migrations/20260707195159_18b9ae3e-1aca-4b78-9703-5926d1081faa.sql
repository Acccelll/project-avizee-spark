CREATE OR REPLACE FUNCTION public.listar_notas_fiscais_ids(p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_tipos text[] DEFAULT NULL::text[], p_status text[] DEFAULT NULL::text[], p_status_sefaz text[] DEFAULT NULL::text[], p_modelos text[] DEFAULT NULL::text[], p_origens text[] DEFAULT NULL::text[], p_fornecedores uuid[] DEFAULT NULL::uuid[], p_clientes uuid[] DEFAULT NULL::uuid[], p_search text DEFAULT NULL::text, p_order_by text DEFAULT 'data_emissao'::text, p_ascending boolean DEFAULT false, p_offset integer DEFAULT 0, p_limit integer DEFAULT 50)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  WITH base AS (
    SELECT n.id, n.data_emissao, n.numero,
           -- Chave de ordenação natural para "numero" (texto): zero-padded 20
           -- para que '10' < '100' < '1000' em vez da ordem lexicográfica.
           lpad(regexp_replace(coalesce(n.numero, ''), '[^0-9]', '', 'g'), 20, '0') AS numero_sort,
           n.valor_total, n.created_at
  ...
  ordered AS (
    SELECT id, ROW_NUMBER() OVER (
      ORDER BY
        ...
        CASE WHEN NOT p_ascending AND v_order_col = 'numero'       THEN numero_sort  END DESC NULLS LAST,
        ...
        CASE WHEN p_ascending AND v_order_col = 'numero'       THEN numero_sort  END ASC NULLS LAST,
