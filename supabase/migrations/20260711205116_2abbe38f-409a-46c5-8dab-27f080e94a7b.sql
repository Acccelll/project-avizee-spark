...
BEGIN
  ...
  CASE p_table
    WHEN 'financeiro_lancamentos' THEN
      SELECT EXISTS(SELECT 1 FROM public.financeiro_lancamentos WHERE id = p_id) INTO v_exists;
      IF NOT v_exists THEN
        RAISE EXCEPTION 'Lançamento financeiro % não encontrado.', p_id
          USING ERRCODE = '22023';
      END IF;

      -- Remove dependências (admin pode forçar mesmo com baixas/conciliações).
      DELETE FROM public.conciliacao_matches WHERE lancamento_id = p_id;
      ...
      UPDATE public.cartao_fatura_lancamentos SET lancamento_id = NULL WHERE lancamento_id = p_id;
      ...
      -- Filhos (parcelas) recursivamente: remove baixas/matches/pares antes.
      DELETE FROM public.conciliacao_matches WHERE lancamento_id IN (SELECT id FROM public.financeiro_lancamentos WHERE documento_pai_id = p_id);
      DELETE FROM public.conciliacao_pares   WHERE lancamento_id IN (SELECT id FROM public.financeiro_lancamentos WHERE documento_pai_id = p_id);
      DELETE FROM public.financeiro_baixas   WHERE lancamento_id IN (SELECT id FROM public.financeiro_lancamentos WHERE documento_pai_id = p_id);
