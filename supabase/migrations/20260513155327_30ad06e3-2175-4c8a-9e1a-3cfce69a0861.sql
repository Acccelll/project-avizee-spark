...
BEGIN
  ...
  -- Bypass admin para NF de entrada: admin pode editar qualquer campo
  -- mesmo em status confirmada/importada/cancelada.
  IF OLD.tipo = 'entrada' AND public.has_role(auth.uid(), 'admin') THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  SELECT status, tipo INTO v_status, v_tipo FROM public.notas_fiscais
    WHERE id = COALESCE(NEW.nota_fiscal_id, OLD.nota_fiscal_id);

  -- Bypass admin para itens de NF de entrada
  IF v_tipo = 'entrada' AND public.has_role(auth.uid(), 'admin') THEN
    RETURN COALESCE(NEW, OLD);
  END IF;
