...
DROP FUNCTION IF EXISTS public.importar_nfe_entrada(jsonb, uuid);
...
DECLARE
  ...
  v_empresa       uuid;
BEGIN
  -- Resolve empresa: parâmetro > current_empresa_id() > primeira empresa cadastrada
  v_empresa := coalesce(
    p_empresa_id,
  ...
  ELSE
    UPDATE public.notas_fiscais SET
      ...
      tipo_operacao     = 'entrada',  -- força entrada
      ...
      origem            = coalesce(nullif(origem,''), nullif(trim(v_nota->>'origem'),''), 'importacao_xml'),
...
GRANT EXECUTE ON FUNCTION public.importar_nfe_entrada(jsonb, uuid) TO authenticated, service_role;