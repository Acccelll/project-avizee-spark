...
export function useImportacaoCadastros() {
  ...
  const generatePreview = useCallback(async () => {
    ...
    try {
      ...
      if (importType === "produtos") {
        const prods = await listProdutosLookup();
        prods.forEach(p => {
          if (p.codigo_legado) existingByLegado.set(p.codigo_legado, p.id);
          if (p.codigo_interno) existingByKey.set(p.codigo_interno, p.id);
        });
      } else {
        const entities = importType === "clientes"
          ? await listClientesLookup()
          : await listFornecedoresLookup();
        entities.forEach(e => {
  ...
  const processImport = async () => {
    ...
    try {
      ...
      if (validos.length > 0) {
        ...
        await insertStagingChunks("stg_cadastros", stagingRows);
      }

      await logImportacao(
        currentLoteId,
        "info",
        `Staging concluído: ${validos.length} válidos (${novos} novos, ${atualizados} atualizações), ${errosCount} erros.`,
      );
  ...
  const finalizeImport = async (loteIdParam?: string) => {
    ...
    setIsProcessing(true);
    try {
      const result = await consolidarCadastros(targetLoteId);
      if (result.erro) {
      ...
      await logImportacao(
        targetLoteId,
        "info",
        `Consolidação concluída: ${result.inseridos ?? 0} inseridos, ${result.atualizados ?? 0} atualizados, ${result.erros ?? 0} erros, ${result.ignorados ?? 0} ignorados.`,
      );
      toast.success(
        `Consolidação concluída: ${result.inseridos ?? 0} inseridos, ${result.atualizados ?? 0} atualizados.`
      );
      return true;
    } catch (error: unknown) {
