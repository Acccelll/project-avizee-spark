...
function readFromStorage(key: string): Layout[] | null {
  try {
    const raw = localStorage.getItem(key);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed) && parsed.length > 0) return parsed as Layout[];
  } catch {
  ...
  const setLayout = useCallback(
    (nextLayout: Layout[]) => {
      ...
      try {
        ...
        // quota exceeded or storage unavailable — proceed without persistence
      }
    },
    [storageKey],
  );
