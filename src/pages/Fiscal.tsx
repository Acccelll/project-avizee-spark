...
const Fiscal = () => {
  ...
  const fornecedoresCrud = useSupabaseCrud<Record<string, unknown>>({ table: "fornecedores" });
  const clientesCrud = useSupabaseCrud<Record<string, unknown>>({ table: "clientes" });
  const produtosCrud = useSupabaseCrud<Record<string, unknown>>({ table: "produtos" });
  const [ordensVenda, setOrdensVenda] = useState<Record<string, unknown>[]>([]);
  const [contasContabeis, setContasContabeis] = useState<Record<string, unknown>[]>([]);
  ...
  const [devolucaoItens, setDevolucaoItens] = useState<Record<string, unknown>[]>([]);
  ...
  const openEdit = async (n: NotaFiscal) => {
    ...
    const loadedItems = (itens || []).map((i: Record<string, unknown>) => ({
    ...
    (itens || []).forEach((i: Record<string, unknown>, idx: number) => {
  ...
  const openDanfe = async (n: NotaFiscal) => {
    ...
    setDanfeData({
      ...
      itens: (itens || []).map((i: Record<string, unknown>) => ({ descricao: i.produtos?.nome || "", quantidade: i.quantidade, valor_unitario: i.valor_unitario, cfop: i.cfop, cst: i.cst, icms_valor: i.icms_valor, ipi_valor: i.ipi_valor, pis_valor: i.pis_valor, cofins_valor: i.cofins_valor })),
  ...
  const handleSaveAndConfirm = async () => {
    ...
    try {
      ...
      await confirmarNotaFiscal({ nf: nfForConfirm as Parameters<typeof confirmarNotaFiscal>[0]["nf"], parcelas });
  ...
  const handleXmlImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (nfe.emitente.cnpj) {
        ...
        const matched = fornecedoresCrud.data.find((f: Record<string, unknown>) => (f.cpf_cnpj || "").replace(/\D/g, "") === cnpjClean);
      ...
      const mappedItems: GridItem[] = nfe.itens.map((nfeItem) => {
        const matchedProd = produtosCrud.data.find((p: Record<string, unknown>) => p.codigo_interno === nfeItem.codigo || p.sku === nfeItem.codigo);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (err: unknown) { logger.error('[fiscal] salvar NF:', err); toast.error("Erro ao salvar nota fiscal."); }
    setSaving(false);
  };
  ...
  const openDevolucao = async (nf: NotaFiscal) => {
    ...
    setDevolucaoNF(nf);
    setDevolucaoItens((itens || []).map((i: Record<string, unknown>) => ({ ...i, qtd_devolver: 0, nome: i.produtos?.nome || "—" })));
      ...
      <FormModal open={modalOpen && mode === "create"} onClose={() => setModalOpen(false)} title="Nova Nota Fiscal" size="xl">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-accent/30 rounded-lg p-4 space-y-3">
            {form.tipo === "entrada" ? (
              <><Label className="text-sm font-semibold">Fornecedor</Label><AutocompleteSearch options={fornecedoresCrud.data.map((f: Record<string, unknown>) => ({ id: f.id, label: f.nome_razao_social, sublabel: f.cpf_cnpj }))} value={form.fornecedor_id} onChange={(id) => setForm({ ...form, fornecedor_id: id })} placeholder="Buscar fornecedor..." /></>
            ) : (
              <><Label className="text-sm font-semibold">Cliente</Label><AutocompleteSearch options={clientesCrud.data.map((c: Record<string, unknown>) => ({ id: c.id, label: c.nome_razao_social, sublabel: c.cpf_cnpj }))} value={form.cliente_id} onChange={(id) => setForm({ ...form, cliente_id: id })} placeholder="Buscar cliente..." /></>
            )}
          </div>
          {form.tipo === "saida" && ordensVenda.length > 0 && (
            <div className="space-y-2"><Label>Pedido (opcional)</Label>
              <Select value={form.ordem_venda_id || "none"} onValueChange={(v) => setForm({ ...form, ordem_venda_id: v === "none" ? "" : v })}><SelectTrigger><SelectValue placeholder="Vincular a um Pedido..." /></SelectTrigger><SelectContent><SelectItem value="none">Nenhum</SelectItem>{ordensVenda.map((ov: Record<string, unknown>) => (<SelectItem key={ov.id} value={ov.id}>{ov.numero} — {ov.clientes?.nome_razao_social || ""}</SelectItem>))}</SelectContent></Select>
            </div>
          )}
              ...
              <div className="space-y-2 rounded-lg border p-3">
                {items.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    ...
                    <Select value={itemContaContabil[idx] || "none"} onValueChange={(v) => setItemContaContabil(prev => ({ ...prev, [idx]: v === "none" ? "" : v }))}><SelectTrigger className="h-8 text-xs flex-1"><SelectValue placeholder="Conta contábil..." /></SelectTrigger><SelectContent><SelectItem value="none">Nenhuma</SelectItem>{contasContabeis.map((c: Record<string, unknown>) => (<SelectItem key={c.id} value={c.id}>{c.codigo} - {c.descricao}</SelectItem>))}</SelectContent></Select>
                  </div>
                ))}
              </div>
            </div>
          ...
          {contasContabeis.length > 0 && (
            <div className="space-y-2"><Label>Conta Contábil Geral (fallback para itens sem conta)</Label>
              <Select value={form.conta_contabil_id || "none"} onValueChange={(v) => setForm({ ...form, conta_contabil_id: v === "none" ? "" : v })}><SelectTrigger><SelectValue placeholder="Vincular conta contábil..." /></SelectTrigger><SelectContent><SelectItem value="none">Nenhuma</SelectItem>{contasContabeis.map((c: Record<string, unknown>) => (<SelectItem key={c.id} value={c.id}>{c.codigo} - {c.descricao}</SelectItem>))}</SelectContent></Select>
            </div>
          )}
