...
interface UseFiscalXmlImportArgs {
  ...
  setParcelasPlano: React.Dispatch<
    React.SetStateAction<Array<{ numero: number; vencimento: string; valor: number }>>
  >;
...
/**
 * Pipeline completo de importação XML para o módulo fiscal:
 *  - upload manual via input (`handleXmlImport`)
 *  - anexar XML a uma NF existente (`handleAnexarXmlChange`)
 *  - integração com `TraducaoXmlDrawer` (confirm/cancel)
 *  - quick-add fornecedor/cliente/produto a partir do XML
 *
 * Mantém o comportamento idêntico ao código original do `Fiscal.tsx`
 * (Etapa 6.3, Pass 2 — extração para hook).
 */
export function useFiscalXmlImport(args: UseFiscalXmlImportArgs) {
  ...
  // Handler do quick-add fornecedor: retoma fluxo XML ou apenas seta no form.
  ...
  /** Aciona o input dedicado a anexar XML em uma NF existente. */
  const triggerAnexarXml = (nf: NotaFiscal) => {
    setAnexarTargetNf(nf);
    setTimeout(() => anexarXmlInputRef.current?.click(), 0);
  };

  /** Reseta estado XML — usado ao fechar o modal de criação. */
  const resetXmlOriginState = () => {
    setXmlOriginInfo(null);
    setTraducaoLinhas([]);
  };

  /** Abre o drawer de tradução em modo edição (a partir do banner). */
  const openTraducaoEdit = () => {
    setTraducaoReadOnly(false);
    setTraducaoOpen(true);
  };

  /** Abre o quick-add de fornecedor a partir do form (cadastro inline). */
  const openQuickFornecedorFromForm = () => {
    setQuickFornecedorDefaults({});
    setQuickFornecedorOpen(true);
  };

  /** Abre quick-add produto pelo botão "+ Novo produto" do ItemsGrid. */
  const openQuickProdutoFromForm = () => {
    setQuickProdutoLinhaIdx(-1);
    setQuickProdutoNome("");
  };
  ...
  return {
    ...
    traducaoOpen,
    traducaoReadOnly,
    handleTraducaoConfirm,
    handleTraducaoCancel,
    openTraducaoEdit,
    // origem / banner
    xmlOriginInfo,
    setXmlOriginInfo,
    resetXmlOriginState,
    // pending state (exposto para drawer)
    pendingXmlImport,
    // quick-add: produto
    quickProdutoLinhaIdx,
    setQuickProdutoLinhaIdx,
    quickProdutoNome,
    setQuickProdutoNome,
    handleQuickProdutoCreated,
    openQuickProdutoFromForm,
    // quick-add: fornecedor
    quickFornecedorOpen,
    setQuickFornecedorOpen,
    quickFornecedorDefaults,
    handleQuickFornecedorCreated,
    openQuickFornecedorFromForm,
    // quick-add: cliente
    quickClienteOpen,
    setQuickClienteOpen,
    quickClienteDefaults,
    handleQuickClienteCreated,
  };
}