...
export async function fetchNotasFiscaisPaged(
  ...
  const { data: rpcData, error: rpcError } = await supabase.rpc(
    "listar_notas_fiscais_ids",
    {
      ...
      p_status_sefaz: params.statusSefaz?.length ? params.statusSefaz : null,
      p_modelos: params.modelos?.length ? params.modelos : null,
      p_origens: params.origens?.length ? params.origens : null,
      p_fornecedores: params.fornecedores?.length ? params.fornecedores : null,
      p_clientes: params.clientes?.length ? params.clientes : null,
