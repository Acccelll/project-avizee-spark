...
async function materializarDoc(doc:CteDistDfeDoc,ambiente:"1"|"2"):Promise<string|null>{
  ...
  if(existente?.id){
    await supabase.from("cte_distribuicao" as never).update({nota_fiscal_id:existente.id,status_processamento:"processado",updated_at:new Date().toISOString()} as never).eq("chave_acesso",doc.chave);
    return existente.id;
  }
  ...
  await supabase.from("cte_distribuicao" as never).upsert({nsu:doc.nsu,chave_acesso:doc.chave,schema_documento:doc.schema,xml_path:path,resumo:(doc.resumo??{}) as Json,status_processamento:"processado",nota_fiscal_id:notaId||null,ambiente,updated_at:new Date().toISOString()} as never,{onConflict:"empresa_id,chave_acesso"} as never);
  return notaId||null;
}
...
export async function sincronizarCteDistDFe(opcoes?:{ambiente?:"1"|"2";maxLotes?:number}):Promise<{novos:number;duplicados:number;ultNSU?:string;maxNSU?:string;cStat?:string;xMotivo?:string;disabled?:boolean;erro?:string}>{
  ...
  const {data:sync}=await supabase.from("cte_distdfe_sync" as never).select("ultimo_nsu,max_nsu,bloqueado_ate").eq("ambiente",ambiente).limit(1).maybeSingle() as never;
  const s=(sync??{}) as {ultimo_nsu?:string;max_nsu?:string;bloqueado_ate?:string|null};
  ...
  for(let i=0;i<max;i++){
    ...
    if(data.cStat==="656"){
      ...
      await supabase.from("cte_distdfe_sync" as never).upsert({ambiente,ultimo_nsu:cursor,max_nsu:data.maxNSU??lastMax??cursor,bloqueado_ate:ate,ultimo_cstat:data.cStat,ultimo_motivo:data.xMotivo,updated_at:new Date().toISOString()} as never,{onConflict:"empresa_id,ambiente"} as never);
    ...
    for(const doc of data.docs??[]){
      ...
      const {data:row}=await supabase.from("cte_distribuicao" as never).select("id,nota_fiscal_id").eq("chave_acesso",doc.chave).limit(1).maybeSingle() as never;
      if(row){duplicados++;continue;}
      await supabase.from("cte_distribuicao" as never).insert({nsu:doc.nsu,chave_acesso:doc.chave,schema_documento:doc.schema,resumo:(doc.resumo??{}) as Json,status_processamento:"recebido",ambiente} as never);
      try{await materializarDoc(doc,ambiente);novos++;}catch(e){await supabase.from("cte_distribuicao" as never).update({status_processamento:"erro",erro:e instanceof Error?e.message:String(e),updated_at:new Date().toISOString()} as never).eq("chave_acesso",doc.chave);}
    }
    cursor=data.ultNSU??cursor; lastMax=data.maxNSU??lastMax;
    await supabase.from("cte_distdfe_sync" as never).upsert({ambiente,ultimo_nsu:cursor,max_nsu:lastMax??cursor,ultima_sincronizacao:new Date().toISOString(),bloqueado_ate:null,ultimo_cstat:data.cStat,ultimo_motivo:data.xMotivo,updated_at:new Date().toISOString()} as never,{onConflict:"empresa_id,ambiente"} as never);
...
export async function resumoInboxCte(){
  const {data,error}=await supabase.from("cte_distribuicao" as never).select("status_processamento"); if(error)throw error;
  const rows=(data??[]) as Array<{status_processamento:string}>; return rows.reduce((acc,r)=>{acc.total++;acc[r.status_processamento]=(acc[r.status_processamento]??0)+1;return acc;},{total:0} as Record<string,number>);
}
